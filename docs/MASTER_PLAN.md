...
> **Actualizado 2026-09-13:** migrado a **Vercel Services** (3 servicios:
> `web`, `api`, `ocr_position` en Docker/Tesseract) desde el modelo clásico
> de un solo `vercel.json`. Verificado en prod: health, signup, login,
> accounts, dashboard, upload boleta (Blob + gpt-5-mini). Eval OCR: **95.5%**.

| Component | Status | Notes |
|-----------|--------|-------|
| **Deployment (Vercel Services: web + api + ocr_position/Docker + Neon + Blob)** | **STABLE** | Migrado 2026-09-12/13 de vercel.json clásico a `services` (necesario para correr Tesseract en Docker); prod verificado 2026-09-13 (`vercel --prod --yes`, health OK). Detalle completo del proceso de migración (sintaxis `entrypoint`, bindings, problemas resueltos uno a uno) en `docs/PLAN_split_v3.md` cont. 14. Config anterior (single-file) documentada en `docs/PLAN_migracion_vercel.md` |
| **Servicio de posición de ítems (`services/ocr_position`)** | STABLE (best-effort) | Tesseract vía binding privado (`OCR_POSITION_URL`), nunca público. Empareja nombre de ítem (ya leído por IA) contra líneas OCR por similitud, buscando hacia adelante sin ventana fija. `_match_best_effort` prueba el OCR a varias escalas (calculadas del tamaño real de cada foto) con la variante de contraste ESTÁNDAR (×1.8/×2.0, ya probada) y, solo si no empareja el 100%, escala a CLAHE (contraste adaptativo local) — probar CLAHE siempre de entrada rompía boletas que ya leían bien (97%→79% en pruebas); con esta escalada: **100% (70/70 ítems, 69/70 con el bug preexistente de bar_autoctono)** en las 9 boletas del eval set. `_row_segments`: agrupa las palabras de cada fila en BLOQUES separados por hueco real (cantidad+nombre por un lado, precio por otro — nunca un solo recuadro que una ambos sombreando el hueco en blanco de por medio), geométrico (por traslape vertical + huecos horizontales), no por idioma/formato; **cada segmento lleva su PROPIO `y0/y1`** (no el del ítem completo — nombre y precio de una misma fila suelen venir de líneas Tesseract con alturas distintas, más marcado en boletas dobladas/arrugadas; bug real reportado en prod 2026-09-14, corregido). **Fotos sacadas en ángulo** (cuadro entero inclinado, no solo boleta doblada — rompía a partir de ~6° el agrupado de "misma fila" de Tesseract y el orden de lectura del emparejamiento): `_detect_skew_angle` (Hough para estimación gruesa robusta + refinamiento por projection-profile en ventana angosta — el projection-profile solo, probado en vivo, queda dominado por la SILUETA del recibo a partir de ~8° y falla) + `_rotate_expand`/`_unrotate_lines` corren Tesseract sobre la foto enderezada y devuelven coordenadas en el marco de la foto original; solo se prueba en el camino difícil (nunca en el rápido), compite en el mismo torneo `_best_of` que escala/CLAHE. Verificado: 100% en 3 fotos rotadas sintéticas (6-12°, antes rompían) sin regresión en el set recto — fixture permanente en `backend/tests/eval/receipts_synthetic_rotated/` + `services/ocr_position/tests/eval_position.py`. Plan diseñado con Fable, implementado y validado por Sonnet (commit `39cad62`, 2026-09-14). Camino lento (escala barata no alcanza el 100%) paraleliza con `ThreadPoolExecutor` el resto de escalas+CLAHE+deskew (mismo resultado exacto, ~2.7x más rápido en el peor caso — evaluado con Fable). Devuelve `bbox_y0/y1` (alto que cubre todos los segmentos, fallback/compat) + `segments: [{x0,x1,y0,y1}, ...]` (uno o más tramos horizontales reales, cada uno con su propia altura) — la banda en el frontend ya no es un solo recuadro a todo el ancho ni comparte una sola altura entre tramos. También devuelve `ocr_lines` (texto crudo de la foto) — reusado por `_suspect_items` en el backend para detectar ítems que el modelo de visión probablemente alucinó, sin gastar tokens de LLM extra (evaluado con Opus). Si falla o no hay match confiable, posición queda `null` y el frontend cae a reparto parejo — nunca bloquea la subida. Ver `docs/PLAN_split_v3.md` cont. 14, 17, 18, 19 y 20 |
...
| OCR (vision path, /upload — recibos + cartolas) | STABLE | `gpt-4o` (`openai_vision_model`), prompt JSON completo (`_RECEIPT_PROMPT`) — sin cambios desde 2026-09-11. Resize a 2000px antes de enviar (`_resize_for_vision`). Fix boleta price-scaling 2026-09-09 (bug del split "$2.150→$1.765"). Fable 5.1 disponible vía `AI_PROVIDER=anthropic` |
| OCR (vision path, /bills — split de cuentas) | STABLE | Camino DEDICADO desde 2026-09-11 (`vision_parse_bill`). Arquitectura de DOS pasos desde 2026-09-11 (cont. 13): paso 1 lectura 100% LIBRE (`_RECEIPT_PROMPT_BILL`, sin pedir ningún formato — probado que cualquier formato, por mínimo que sea, empeora la precisión en boletas difíciles); paso 2 reformateo solo-texto a `cantidad|nombre|total` (`_REFORMAT_PROMPT_BILL`, `gpt-4.1-mini`, ~2-3s, verificado fiel). `openai_vision_model_bill = "gpt-5.6-luna"` (elegido tras comparar toda la escalera de modelos de barato a caro por pedido explícito del usuario — únicos que acertaron un caso difícil de columnas desalineadas: familia gpt-5.6). **Escalamiento a `gpt-5-mini`** (`openai_vision_model_fallback`) cuando el retry por descuadre de suma dispara (umbral 6%). 8/9 boletas reales del eval exactas; 1 límite conocido chico (línea de descuento a veces perdida en reformateo, `lider_quilicura`, ~2.4%). Historial completo de pruebas (7+ modelos comparados) en `docs/PLAN_split_v3.md` |
...
| Bill splitter (`bills.py` + `/split`) | STABLE | 2026-09-09: arreglado bug "no se guardó el monto" (`account_svc` ImportError en finalize); +toggle "guardar como gasto", +historial "Divisiones guardadas", +pagador por % (Splitwise), +reparto avanzado por ítem (⚙ %/unidades fraccionales/montos). Resaltado de color por ítem sobre la foto, mismo hue que la fila de la lista (`itemColor(idx)`) con `itemHighlightColor(idx)` (más saturado) + borde; posición y tamaño (alto + uno o más TRAMOS horizontales separados, `segments`, no un solo recuadro que une nombre y precio sombreando el hueco de por medio) vienen del servicio `ocr_position` o caen a reparto parejo/tamaño estimado; arrastre manual limpia bbox/segments al corregir. Paso de recorte en DOS ETAPAS antes de subir la foto: primero seleccionar/recortar (recuadro arrastrable + zoom/pan + grilla de referencia), después girar/enderezar sobre el resultado ya recortado (90°/fino ±45°) — "Volver a recortar" restaura la foto original para empezar de cero; todo en canvas del navegador, cero cambios de backend. Nombre/fecha editables al final del flujo. Mensaje de WhatsApp con detalle completo (ítems+valores+quién consumió, quién pagó, balance), reutiliza los mismos cálculos que el desglose en pantalla. `needs_review` por ítem (borde ámbar + aviso en la UI) cuando el nombre+precio que dijo el modelo de visión no aparece en el texto real de la foto — probable alucinación, detectado gratis con el mismo Tesseract del posicionamiento (ver fila de arriba); dispara además el reintento con el modelo de fallback. (Se probó y se revirtió un modo "tap-to-spotlight" — el usuario lo rechazó. También se probó y descartó pedirle bbox a la IA de visión, colorear la foto con IA generativa, agrandar siempre a una resolución fija, usar el precio como señal de matching en el servicio de posición, y aplicar CLAHE siempre en vez de como escalón — regresión confirmada en pruebas A/B, ver `docs/PLAN_split_v3.md` cont. 14/17/18/19.) Historial completo en `docs/PLAN_split_v3.md`. Checkpoint de rollback: tag git `checkpoint-2026-09-11-pre-markers` |
