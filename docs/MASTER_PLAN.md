...
| **Servicio de posición de ítems (`services/ocr_position`)** | STABLE (best-effort) | Tesseract vía binding privado (`OCR_POSITION_URL`), nunca público. Empareja nombre de ítem (ya leído por IA) contra líneas OCR por similitud, buscando hacia adelante sin ventana fija. `_match_best_effort` prueba el OCR a varias escalas (calculadas del tamaño real de cada foto) con la variante de contraste ESTÁNDAR (×1.8/×2.0, ya probada) y, solo si no empareja el 100%, escala a CLAHE (contraste adaptativo local) — probar CLAHE siempre de entrada rompía boletas que ya leían bien (97%→79% en pruebas); con esta escalada: **100% (70/70 ítems)** en las 9 boletas del eval set. `_expand_to_full_row`: agranda el ancho de cada match para cubrir cantidad+nombre+precio cuando Tesseract los separó en bloques de texto distintos (geométrico, por traslape vertical — no por idioma/formato). Camino lento (escala barata no alcanza el 100%) paraleliza con `ThreadPoolExecutor` el resto de escalas+CLAHE (mismo resultado exacto, ~2.7x más rápido en el peor caso — evaluado con Fable). Devuelve `bbox_x0/y0/x1/y1` reales — la banda en el frontend ya no es siempre a todo el ancho/alto. Si falla o no hay match confiable, posición queda `null` y el frontend cae a reparto parejo — nunca bloquea la subida. Ver `docs/PLAN_split_v3.md` cont. 14, 17 y 18 |
...
| Bill splitter (`bills.py` + `/split`) | STABLE | 2026-09-09: arreglado bug "no se guardó el monto" (`account_svc` ImportError en finalize); +toggle "guardar como gasto", +historial "Divisiones guardadas", +pagador por % (Splitwise), +reparto avanzado por ítem (⚙ %/unidades fraccionales/montos). Resaltado de color por ítem sobre la foto, mismo hue que la fila de la lista (`itemColor(idx)`) con `itemHighlightColor(idx)` (más saturado) + borde; posición y tamaño de cada banda (alto Y ancho, cubre cantidad+nombre+precio) vienen del servicio `ocr_position` o caen a reparto parejo/tamaño estimado; arrastre manual limpia el bbox al corregir. Paso de recorte en DOS ETAPAS antes de subir la foto: primero seleccionar/recortar (recuadro arrastrable + zoom/pan + grilla de referencia), después girar/enderezar sobre el resultado ya recortado (90°/fino ±45°) — "Volver a recortar" restaura la foto original para empezar de cero; todo en canvas del navegador, cero cambios de backend. Nombre/fecha editables al final del flujo. Mensaje de WhatsApp con detalle completo (ítems+valores+quién consumió, quién pagó, balance), reutiliza los mismos cálculos que el desglose en pantalla. (Se probó y se revirtió un modo "tap-to-spotlight" — el usuario lo rechazó. También se probó y descartó pedirle bbox a la IA de visión, colorear la foto con IA generativa, agrandar siempre a una resolución fija, usar el precio como señal de matching, y aplicar CLAHE siempre en vez de como escalón — las últimas tres con regresión confirmada en pruebas A/B, ver `docs/PLAN_split_v3.md` cont. 14/17/18.) Historial completo en `docs/PLAN_split_v3.md`. Checkpoint de rollback: tag git `checkpoint-2026-09-11-pre-markers` |
