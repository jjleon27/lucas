...
| **Servicio de posición de ítems (`services/ocr_position`)** | STABLE (best-effort) | Tesseract vía binding privado (`OCR_POSITION_URL`), nunca público. Empareja nombre de ítem (ya leído por IA) contra líneas OCR por similitud, buscando hacia adelante sin ventana fija. 2026-09-14: `_match_best_effort` prueba el OCR a varias escalas (calculadas del tamaño real de cada foto, no fijas) y se queda con la que empareja más ítems — una resolución fija arregla fotos chicas pero rompe fotos que ya estaban bien; verificado 45/47 ítems (95.7%) en 9 boletas reales del eval set. Devuelve también `bbox_x0/x1` (ancho real del texto, no solo alto) — la banda en el frontend ya no es siempre a todo el ancho. Si falla o no hay match confiable, posición queda `null` y el frontend cae a reparto parejo — nunca bloquea la subida. Precisión de Tesseract sobre fotos reales es parcial, aceptado como límite conocido (confirmado: una boleta con la columna de nombres genuinamente ilegible a cualquier escala no logra match). Ver `docs/PLAN_split_v3.md` cont. 14 y cont. 17 |
...
| Bill splitter (`bills.py` + `/split`) | STABLE | 2026-09-09: arreglado bug "no se guardó el monto" (`account_svc` ImportError en finalize); +toggle "guardar como gasto", +historial "Divisiones guardadas", +pagador por % (Splitwise), +reparto avanzado por ítem (⚙ %/unidades fraccionales/montos). 2026-09-11: compartir por WhatsApp vía `navigator.share`; resaltado de color por ítem sobre la foto, mismo hue que la fila de la lista (`itemColor(idx)`) pero con `itemHighlightColor(idx)` (más saturado, tipo tinta de marcador) + borde. Posición y tamaño de cada banda vienen del servicio `ocr_position` (Tesseract, bbox real x0/y0/x1/y1) o caen a reparto parejo/tamaño estimado; arrastre manual para corregir sigue disponible (limpia el bbox al corregir). 2026-09-13/14: paso de recorte manual antes de subir la foto (arrastrable, + girar 90°/enderezado fino ±45° con slider, todo en canvas del navegador) — menos ruido de fondo = mejor OCR; nombre/fecha editables al final del flujo para reconocer la división después. Altura Y ancho de banda ahora ajustados al tamaño real del texto (antes: altura fija/estimada, ancho siempre a toda la foto) — el servicio de posición prueba el OCR a varias escalas por foto (no una fija) y se queda con la que más ítems logra emparejar; 95.7% en 9 boletas reales del eval set. (Se probó y se revirtió un modo "tap-to-spotlight" con zoom automático — el usuario lo rechazó explícitamente. También se probó y descartó pedirle bbox a la IA de visión, colorear la foto con IA generativa, agrandar siempre la foto a una resolución fija, y usar el precio como señal de matching — los últimos dos con regresión confirmada en pruebas A/B, ver `docs/PLAN_split_v3.md` cont. 14/17.) Historial completo en `docs/PLAN_split_v3.md`. Checkpoint de rollback: tag git `checkpoint-2026-09-11-pre-markers` |
