...
| OCR (vision path) | STABLE | `gpt-4o` default desde 2026-09-11 (antes `gpt-4.1`, antes `gpt-5-mini`) — probado contra gpt-5/gpt-4.1 con eval real: ningún modelo es siempre rápido Y preciso (gpt-5 lento Y se equivoca igual; gpt-4o 88.9% eval vs gpt-4.1 92.7%, usuario lo eligió igual tras ver el trade-off). Resize a 2000px antes de enviar (`_resize_for_vision`, antes sin usar). **Escalamiento a `gpt-5-mini`** (`openai_vision_model_fallback`) cuando el retry por descuadre de suma dispara (umbral 6%) — red de seguridad para ítems saltados en boletas complejas, se mantiene sea cual sea el modelo principal. Fix boleta price-scaling 2026-09-09 (bug del split "$2.150→$1.765"). Fable 5.1 disponible vía `AI_PROVIDER=anthropic` |
