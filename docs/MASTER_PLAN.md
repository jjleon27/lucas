...
| OCR (vision path, /bills — split de cuentas) | STABLE | Camino DEDICADO desde 2026-09-11 (`vision_parse_bill`, `_RECEIPT_PROMPT_BILL`) — prompt de texto plano MÍNIMO (sin JSON, sin categoría/moneda/bbox, ni reglas extra — se probó y confirmó que más reglas = peor lectura) en vez de compartir el JSON complejo de /upload. `openai_vision_model_bill = "gpt-4.1-mini"` (el más barato de la familia, no el más grande — probado 3/3 exacto en la boleta más difícil del set). 7/9 boletas reales del eval: 2.3-4.6s sin reintento. **Escalamiento a `gpt-5-mini`** (`openai_vision_model_fallback`, compartido con /upload) cuando el retry por descuadre de suma dispara (umbral 6%) — red de seguridad para ítems saltados. Historial completo de pruebas (7+ modelos comparados) en `docs/PLAN_split_v3.md` |
