...
| **Servicio de posición de ítems (`services/ocr_position`)** | STABLE (best-effort) | Tesseract vía binding privado (`OCR_POSITION_URL`), nunca público. Empareja nombre de ítem (ya leído por IA) contra líneas OCR por similitud, buscando hacia adelante sin ventana fija. `_match_best_effort` prueba el OCR a varias escalas (calculadas del tamaño real de cada foto) con la variante de contraste ESTÁNDAR (×1.8/×2.0, ya probada) y, solo si no empareja el 100%, escala a CLAHE (contraste adaptativo local) — probar CLAHE siempre de entrada rompía boletas que ya leían bien (97%→79% en pruebas); con esta escalada: **100% (70/70 ítems)** en las 9 boletas del eval set. `_row_segments`: agrupa las palabras de cada fila en BLOQUES separados por hueco real (cantidad+nombre por un lado, precio por otro — nunca un solo recuadro que una ambos sombreando el hueco en blanco de por medio), geométrico (por traslape vertical + huecos horizontales), no por idioma/formato. Camino lento (escala barata no alcanza el 100%) paraleliza con `ThreadPoolExecutor` el resto de escalas+CLAHE (mismo resultado exacto, ~2.7x más rápido en el peor caso — evaluado con Fable). Devuelve `bbox_y0/y1` (alto real) + `segments: [{x0,x1}, ...]` (uno o más tramos horizontales reales) — la banda en el frontend ya no es un solo recuadro a todo el ancho. También devuelve `ocr_lines` (texto crudo de la foto) — reusado por `_suspect_items` en el backend para detectar ítems que el modelo de visión probablemente alucinó, sin gastar tokens de LLM extra (evaluado con Opus). Si falla o no hay match confiable, posición queda `null` y el frontend cae a reparto parejo — nunca bloquea la subida. Ver `docs/PLAN_split_v3.md` cont. 14, 17, 18 y 19 |
